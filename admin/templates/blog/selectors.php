...
function showCategory( obj ) {
	var cat = $( obj ).val();
	if ( cat !== "" )
		window.location.href = '<?php echo basename($_SERVER['PHP_SELF']) ?>?category=' + cat;
	else
...
function showPost( obj, objcat ) {
	var post = $( obj ).val(),
		cat = $( objcat ).val();
	if ( post !== "" && cat !== "" )
		window.location.href = '<?php echo basename($_SERVER['PHP_SELF']) ?>?category=' + cat + '&post=' + post;
	else
		window.location.href = '<?php echo basename($_SERVER['PHP_SELF']) ?>';	
}
</script>
...
<?php foreach($categories as $category => $post): ?>
		<option value="<?php echo $category ?>"<?php echo $category == $selectedCategory ? " selected" : "" ?>><?php echo str_replace("_", " ", $category) ?></option>
<?php endforeach; ?>
	</select>
